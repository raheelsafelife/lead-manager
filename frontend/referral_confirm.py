...
def display_referral_confirm(lead, db, highlight=False):
    ...
    with st.expander(expander_title, expanded=highlight):
        ...
        with col_start:
            if st.button("Care Start", key=f"care_start_{lead.id}", type="primary", use_container_width=True, disabled=(lead.care_status == "Care Start")):
        ...
        with col_not_start:
            if st.button("Not Start", key=f"not_start_{lead.id}", type="secondary", use_container_width=True, disabled=(lead.care_status == "Not Start")):
                update_data = LeadUpdate(
