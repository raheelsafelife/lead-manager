...
RUN python3 -c 'import streamlit; import os; p = os.path.join(os.path.dirname(streamlit.__file__), "static", "index.html"); html = open(p).read(); html = html.replace("<title>Streamlit</title>", "<title>Lead Manager</title>"); html = html.replace("favicon.png", "favicon.png?v=8"); script = "<script>const force = () => { if (document.title !== \"Lead Manager\") document.title = \"Lead Manager\"; document.querySelectorAll(\"link[rel*=\\\"icon\\\"]\").forEach(l => { if (!l.href.includes(\"favicon.png?v=8\")) l.href = \"./favicon.png?v=8\"; }); }; force(); const obs = new MutationObserver(force); obs.observe(document.documentElement, { childList: true, subtree: true, attributes: true }); setTimeout(() => obs.disconnect(), 60000); </script>"; html = html.replace("<head>", "<head>" + script) if "MutationObserver" not in html else html; open(p, "w").write(html)' && \
